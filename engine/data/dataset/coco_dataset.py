...
class CocoDetection(torchvision.datasets.CocoDetection, DetDataset):
    ...
    def __init__(self, img_folder, ann_file, transforms, return_masks=False, remap_mscoco_category=False, use_domain_label=False):
    ...
    def __getitem__(self, idx):
        ...
        # print("use_domain_label:", self.use_domain_label)
        # ======= ⭐ 在这里加 =======
        if self.use_domain_label:
            if "labels" in target and len(target["labels"]) > 0:
                label = target["labels"][0].item()
                B_DOMAIN_IDS = [0, 1, 2, 3, 4]
                if label in B_DOMAIN_IDS:
                    domain_label = 1  # B显
                else:
                    domain_label = 0  # RGB
            else:
                domain_label = 0  # 默认（防止空图）

            target["domain_label"] = domain_label
        # ==========================
