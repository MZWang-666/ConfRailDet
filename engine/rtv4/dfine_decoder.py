...
from .confusion_head import DomainGuidedConfusionHead
    ...
    def forward(self,
        ...
        for i, layer in enumerate(self.layers):
            ...
            if self.training or i == self.eval_idx:
                ...
                dec_out_query_features.append(output)
        ...
        return torch.stack(dec_out_bboxes), torch.stack(dec_out_logits), \
               ...
               torch.stack(dec_out_query_features), pre_bboxes, pre_scores
...
class DFINETransformer(nn.Module):
    ...
    def __init__(self,
                 ...
                 mlp_act='relu',
                  ...
                  class_counts=None,
                  cb_iqs_alpha=0.5,
                  cb_iqs_score_type='mul',
                  use_confusion_head=False,
                  confusion_domain_class_ids=None,
                  confusion_temperature=0.2,
                  confusion_logit_alpha=0.25,
                  confusion_fusion_mode='add',
                  confusion_use_domain_context=False,
                  confusion_context_hidden_dim=None,
                  ):
        super().__init__()
        ...
        self.reg_max = reg_max
        self.use_cb_iqs = use_cb_iqs
        self.cb_iqs_alpha = float(cb_iqs_alpha)
        assert cb_iqs_score_type in ('mul', 'add_log'), ''
        self.cb_iqs_score_type = cb_iqs_score_type
        cb_iqs_class_weight = self._build_cb_iqs_class_weight(class_counts, num_classes)
        self.register_buffer('cb_iqs_class_weight', cb_iqs_class_weight, persistent=False)
        self.use_confusion_head = bool(use_confusion_head)
      ...
      # denoising
        ...
        if self.use_confusion_head:
            self.confusion_head = DomainGuidedConfusionHead(
                num_classes=num_classes,
                hidden_dim=hidden_dim,
                domain_class_ids=confusion_domain_class_ids,
                temperature=confusion_temperature,
                logit_alpha=confusion_logit_alpha,
                fusion_mode=confusion_fusion_mode,
                use_domain_context=confusion_use_domain_context,
                context_hidden_dim=confusion_context_hidden_dim,
            )
        else:
            self.confusion_head = None
    ...
    @classmethod
    def _build_cb_iqs_class_weight(cls, class_counts, num_classes):
        counts = cls._as_class_count_tensor(class_counts, num_classes)
        if counts is None:
            return None
    ...
    def _get_cb_iqs_scores(self, outputs_logits):
        if not self.use_cb_iqs or self.cb_iqs_class_weight is None or self.query_select_method == 'agnostic':
            return outputs_logits

        class_weight = self.cb_iqs_class_weight.to(device=outputs_logits.device, dtype=outputs_logits.dtype)
        class_weight = class_weight.clamp_min(1e-12).view(1, 1, -1)

        if self.cb_iqs_score_type == 'add_log':
            return outputs_logits + self.cb_iqs_alpha * class_weight.log()

        return outputs_logits.sigmoid() * class_weight.pow(self.cb_iqs_alpha)

    def _apply_confusion_head(self, out_logits, out_query_features, domain_context):
        if self.confusion_head is None:
            return out_logits, None

        fused_logits, proto_logits = [], []
        for layer_logits, layer_features in zip(out_logits, out_query_features):
            fused, proto = self.confusion_head(layer_features, layer_logits, domain_context=domain_context)
            fused_logits.append(fused)
            proto_logits.append(proto)

        return torch.stack(fused_logits), torch.stack(proto_logits)
    ...
    def _select_topk(self, memory: torch.Tensor, outputs_logits: torch.Tensor, outputs_anchors_unact: torch.Tensor, topk: int):
        select_scores = self._get_cb_iqs_scores(outputs_logits)

        if self.query_select_method == 'default':
            _, topk_ind = torch.topk(select_scores.max(-1).values, topk, dim=-1)

        elif self.query_select_method == 'one2many':
            _, topk_ind = torch.topk(select_scores.flatten(1), topk, dim=-1)
            topk_ind = topk_ind // self.num_classes

        elif self.query_select_method == 'agnostic':
            _, topk_ind = torch.topk(select_scores.squeeze(-1), topk, dim=-1)
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        out_bboxes, out_logits, out_corners, out_refs, out_query_features, pre_bboxes, pre_logits = self.decoder(
            init_ref_contents,
        ...
        out_proto_logits = None
        if self.confusion_head is not None:
            out_logits, out_proto_logits = self._apply_confusion_head(
                out_logits,
                out_query_features,
                domain_context=memory.mean(dim=1),
            )
        ...
        if self.training and dn_meta is not None:
            ...
            _, out_query_features = torch.split(out_query_features, dn_meta['dn_num_split'], dim=2)
            if out_proto_logits is not None:
                _, out_proto_logits = torch.split(out_proto_logits, dn_meta['dn_num_split'], dim=2)
        ...
        if out_proto_logits is not None:
            out['confusion_proto_logits'] = out_proto_logits[-1]
            out['confusion_query_features'] = out_query_features[-1]

        if self.training and self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss2(out_logits[:-1], out_bboxes[:-1], out_corners[:-1], out_refs[:-1],
                                                     out_corners[-1], out_logits[-1],
                                                     confusion_proto_logits=out_proto_logits[:-1] if out_proto_logits is not None else None,
                                                     confusion_query_features=out_query_features[:-1] if out_proto_logits is not None else None)
    ...
    def _set_aux_loss2(self, outputs_class, outputs_coord, outputs_corners, outputs_ref,
                       ...
                       confusion_proto_logits=None, confusion_query_features=None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        aux_outputs = []
        for i, (a, b, c, d) in enumerate(zip(outputs_class, outputs_coord, outputs_corners, outputs_ref)):
            ...
            if confusion_proto_logits is not None:
                item['confusion_proto_logits'] = confusion_proto_logits[i]
            if confusion_query_features is not None:
                item['confusion_query_features'] = confusion_query_features[i]
            aux_outputs.append(item)
        return aux_outputs
