...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0):
        ...
        self.alpha = alpha
        self.gamma = gamma
        self.diw_apply_to_matcher_cost = False
        self.register_buffer('diw_cost_weight', None, persistent=False)

        assert self.cost_class != 0 or self.cost_bbox != 0 or self.cost_giou != 0, "all costs cant be 0"

    def set_diw_cost_weight(self, enabled=False, class_weight=None):
        self.diw_apply_to_matcher_cost = bool(enabled)
        if enabled and class_weight is not None:
            self.diw_cost_weight = class_weight.detach().float().cpu()
        else:
            self.diw_cost_weight = None
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False):
        ...
        if self.diw_apply_to_matcher_cost and self.diw_cost_weight is not None and tgt_ids.numel() > 0:
            diw_cost_weight = self.diw_cost_weight.to(device=cost_class.device, dtype=cost_class.dtype)
            target_weight = torch.ones(tgt_ids.shape, device=cost_class.device, dtype=cost_class.dtype)
            valid = (tgt_ids >= 0) & (tgt_ids < diw_cost_weight.numel())
            if valid.any():
                target_weight[valid] = diw_cost_weight[tgt_ids[valid]]
                cost_class = cost_class * target_weight.unsqueeze(0)
