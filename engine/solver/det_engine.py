...
def train_one_epoch(self_lr_scheduler, lr_scheduler, model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        targets = [
            {
                k: (v.to(device) if hasattr(v, 'to') else torch.as_tensor(v, device=device))
                for k, v in t.items()
            }
            for t in targets
        ]
...
def evaluate(model: torch.nn.Module, criterion: torch.nn.Module, postprocessor, data_loader, coco_evaluator: CocoEvaluator,
             device, output_dir=None, print_per_class_ap=False, per_class_ap_filename="per_class_ap.csv"):
    model.eval()
    criterion.eval()
    ...
    if coco_evaluator is not None:
        ...
        if print_per_class_ap and 'bbox' in iou_types and dist_utils.is_main_process():
            output_file = str(output_dir / per_class_ap_filename) if output_dir is not None else None
            coco_evaluator.summarize_per_class('bbox', output_file=output_file)
